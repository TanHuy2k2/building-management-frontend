  ...
  const handleCreateEvent = async () => {
    ...
    if (formData.facility_reservation_id) {
      ...
      if (reservation) {
        ...
        if (facility) {
          const buildingRes = await getBuildingByIdApi(facility.building_id);
          if (buildingRes.success) {
            const buildingName = buildingRes.data.name;
            location = `${facility.name} (${buildingName})`;
          } else {
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(value: string) =>
                    setFormData({ ...formData, facility_reservation_id: value })
                  }
                >
                  <SelectTrigger>
